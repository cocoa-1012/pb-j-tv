...
$total_play = 0;
	...
	if($qBcount>0){ $querysRes = mysql_fetch_object($queryB); // echo
$querysRes->camera_name; $cam_ips = $querysRes->ip_address; }else{ $cam_ips='';
} ?>
<!DOCTYPE html>
...
<head>
    ...
    function show_data(mid) {
        ...
        $('.btn').load('<?php print $projecturl; ?>alert.php?uid=<?php echo $userid;?>&msg_id=' + mid + '').hide()
            .fadeIn(30000);
    }
        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                for (k in allmessages) {
                    ...
                    html = '';
                    html = html + '<div id="message-' + m.id + '" class="message message-type-' + m
                        .status_type + ' ' + chars_class + ' cant_' + countTexts + '">';
                    ...
                    clicksound = ss_soundbits("<?= SOUND_TRACK_PATH ?>/" + m.sound + ".ogg",
                        "<?= SOUND_TRACK_PATH ?>/" + m.sound, "<?= SOUND_TRACK_PATH ?>/" + m.sound +
                        ".mp3", "<?= SOUND_TRACK_PATH ?>/" + m.sound + ".wav",
                        "<?= SOUND_TRACK_PATH ?>/Jubilant Fanfare.wav");
    ...
    (function($) {
        $(document).ready(function() {
            ...
            var refreshId = setInterval(function() {
                container.load(
                    '<?php print $projecturl; ?>content_updates.php?front_userid=<?php print $userid; ?>&total_play=<?php print $total_play; ?>'
                );
            }, 20000);
            ...
            !--content_social.load(
                "<?php print $projecturl; ?>content_social.php?front_userid=<?php print $userid; ?>");
            -- >
            var refreshId = setInterval(function() {
                content_social.load(
                    '<?php print $projecturl; ?>content_social.php?front_userid=<?php print $userid; ?>&projecturl=<?php print $projecturl; ?>'
                );
            }, 50000);
        });
    })(jQuery);
    ...
    function updateClock() {
        ...
        var currentTimeString = currentHours + "<span class='clocktoggle'>:</span>" + currentMinutes +
            /*":" + currentSeconds + */
            " " + timeOfDay;
...
top:45px !important;
    ...
    .new .display .main iframe {
        width: 100%;
        height: 100%;
        border: 0;
        background: url(<?php print $projecturl; ?>admin/images/bg_pattern.png);
    }
    ...
    .new #messages .message .message-title {
        padding: 10px 20px;
        color: #fff;
        text-align: center;
        font-size: 25px;
    ...
    .new #messages .message .message-title .message-time {
        float: right;
    ...
    .new #messages .message .message-text {
        padding: 20px 30px;
        font-size: 35px;
        min-height: 150px;
    }

    .new #messages .message.chars-100 .message-text {
        font-size: 40px;
    }

    .new #messages .message.chars-80 .message-text {
        font-size: 50px;
    }

    .new #messages .message.chars-50 .message-text {
        font-size: 60px;
    }

    .new #messages .message.chars-20 .message-text {
        font-size: 70px;
    }

    .new #messages .message.chars-0 .message-text {
        font-size: 100px;
    }
    ...
    .new #messages .message.message-type-1 .message-title {
        background: DarkBlue;
    }

    .new #messages .message.message-type-2 .message-title {
        background: forestgreen;
    }

    .new #messages .message.message-type-3 .message-title {
        background: firebrick;
    }

    .new #messages .message.message-type-4 .message-title {
        background: blueviolet;
    }

    .new #messages .message.message-type-5 .message-title {
    ...
    .new.has_messages .display .sidebar .logo {
        bottom: 45%;
    }
...
<body class="new total_0">
    <div class="display">
        <div class="main">
            <div id="messages"></div>
            <iframe id="livePlayer" width="100%" height="100%" scrolling="no"
                src="<?php echo$projecturl?>camBack.html?<?php echo$cam_ips?>" allowtransparency="true"
                seamless></iframe>
        </div>
        <div class="sidebar">
            <div id="clock"></div>
